...
import Energistics.Etp.v12.Protocol.SupportedTypes.GetSupportedTypes;
import Energistics.Etp.v12.Protocol.SupportedTypes.GetSupportedTypesResponse;
...
public class SupportedTypesHandler extends ProtocolHandler{
	public static final Logger logger = LogManager.getLogger(SupportedTypesHandler.class);

    public static final CommunicationProtocol protocol = CommunicationProtocol.SUPPORTED_TYPES;
    ...
    public Collection<Message> on_GetSupportedTypesResponse(GetSupportedTypesResponse msg, MessageHeader msgHeader, ClientInfo clientInfo) {
        Collection<Message> res = new ArrayList<>();
        Message _msg = new ETPError.NotSupportedError().to_etp_message(msgHeader.getMessageId());
        logger.error(_msg);
        res.add(_msg);
